...
class MarkdownWithColorAIO(html.Div):  # html.Div will be the "parent" component
    ...
    # not any more ...
    class ids:
        dropdown = lambda aio_id: 'MarkdownWithColorAIO-dropdown-' + aio_id
        markdown = lambda aio_id: 'MarkdownWithColorAIO-markdown-' + aio_id

    # Make the ids class a public class
    ids = ids

    @classmethod
    def dropdown_id(cls, aio_id):
        return cls.ids.dropdown(aio_id)
    ...
    def __init__(
        ...
        @callback(
            Output(self.ids.markdown(aio_id), 'style'),
            Input(self.ids.dropdown(aio_id), 'value'),
            State(self.ids.markdown(aio_id), 'style'),
        )
